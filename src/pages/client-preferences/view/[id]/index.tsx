...
import { getClientPreferenceById } from 'apiSdk/client-preferences';
...
function ClientPreferenceViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ClientPreferenceInterface>(
    ...
    () =>
      getClientPreferenceById(id, {
        relations: ['client', 'news_category'],
      }),
  );
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Client Preference Detail View
          </Text>
          {hasAccess('client_preference', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/client-preferences/edit/${data?.id}`} passHref legacyBehavior>
              <Button
